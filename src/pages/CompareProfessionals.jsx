...
function CompareProfessionals() {
  ...
  // IDs originales seleccionados para filtrar al volver a pedir favoritos
  ...
  }, []); // Solo al montar — location.state no cambia

  // Zonas únicas de todos los profesionales
  ...
  // Filtro de zona aplicado sobre allProfessionals
  useEffect(() => {
    if (zonaFilter.length === 0) {
      ...
      setProfessionals(
        allProfessionals.filter(p =>
          (p.zones || []).some(z => zonaFilter.includes(`${z.zona}, ${z.provincia}`))
        )
      );
    }
  ...
  const getDisplayedStats = (prof) => {
    ...
    return work
      ? { avgScore: work.avgScoreInPeriod, totalRatings: work.ratingsCountInPeriod }
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-gradient-to-br from-purple-500 to-pink-600 px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-3xl roboto-light text-white">
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-8 pb-4">
                  ...
                  {allZones.map((zone, i) => {
                    ...
                    return (
                      <button
                        key={i}
                        onClick={() => {
                          setZonaFilter(isActive
                            ? zonaFilter.filter(z => z !== key)
                            : [...zonaFilter, key]
                          );
                        }}
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div className="text-left">
                ...
                <span className="text-xs text-gray-400">
                  {quickFilter
                    ? `Mostrando calificaciones de los últimos ${
                        { last30: '30 días', last3m: '3 meses', last6m: '6 meses', last12m: '12 meses' }[quickFilter]
                      }`
                    : 'Solo calificaciones recibidas en el período elegido'}
                </span>
              </div>
            </div>
            {showDateFilter
        ...
        {/* Sin resultados tras filtrar por zona */}
