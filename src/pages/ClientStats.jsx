...
function ClientStats() {
  ...
  const calculateBadges = (ratingsData) => {
    ...
    if (average >= 4.5) earnedBadges.push({ icon: '🌟', name: 'Generoso', description: 'Promedio mayor a 4.5 estrellas', unlocked: true, special: true });
      ...
      <div className="bg-gradient-to-br from-green-500 to-teal-600 px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-white/90 animate-slideUp delay-100">
            Tu actividad y logros como calificador
          </p>
        </div>
      </div>

      {/* Contenedor principal con separación corregida (-mt-4) */}
      <div className="max-w-4xl mx-auto px-4 -mt-4 pb-8">
        
        {/* Quick Stats - Los dos cuadrados principales */}
              ...
              <div className="absolute left-10 top-0 right-2 bottom-10">
                <svg className="w-full h-full" viewBox="0 0 500 160" preserveAspectRatio="none">
                  {stats.monthlyActivity.map((m, i) => {
                    const maxCount = Math.max(...stats.monthlyActivity.map(item => item.count), 1);
